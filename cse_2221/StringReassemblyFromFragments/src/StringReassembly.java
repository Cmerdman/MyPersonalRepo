...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //returns str1 plus a substring of str2 sub the overlap
        return str1.concat(str2.substring(overlap, str2.length()));
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        /*
         * Note: Precondition not checked! It's okay though by means of design
         * by contract
         */

        //uses contains to check if str is in strSet already
        if (!(strSet.contains(str))) {
            int check = 0;
            //uses check to see if s contains str for all s in set
            for (String s : strSet) {
                if (s.contains(str)) {
                    check = 1;
                }
            }
            Stack<String> stringsToRemove = new Stack1L<>();
            //if check is still 0 then str was never a substring of any s in set
            if (check == 0) {
                //go through the set again to see if str contains any s in set
                for (String s : strSet) {
                    if (str.contains(s)) {
                        //if it does add to a stack and remove after for loop
                        stringsToRemove.push(s);
                    }
                }
                while (stringsToRemove.length() != 0) {
                    strSet.remove(stringsToRemove.pop());
                }
                //finally add str
                strSet.add(str);
            }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // while the input has lines, read them and add to the set by using method
        while (!(input.atEOS())) {
            String s = input.nextLine();
            //method checks to make sure that we aren't adding substrings
            addToSetAvoidingSubstrings(in, s);
        }
        return in;
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        // for all character in the text check if it is '~'
        // and if so print a line, else print the character
