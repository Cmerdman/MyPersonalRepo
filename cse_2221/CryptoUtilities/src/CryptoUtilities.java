...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        if (!(m.isZero())) {
            //if m isn't zero then assign n to the remainder of n / m
            // and repeat until m is zero
            n.transferFrom(n.divide(m));
            reduceToGCD(m, n);
        } else {
            //if m is zero then the greatest common divisor is always n
            NaturalNumber t = n.newInstance();
            t.copyFrom(n);
            n.copyFrom(m);
            m.copyFrom(t);
            t.clear();
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        nCopy.copyFrom(n);
        //if temp can be an int and (int)temp modulus 2 is 0 then n is even
        int temp = nCopy.divideBy10();
        if (temp % 2 == 0) {
            ret = true;
        }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        pCopy.copyFrom(p);
        /*
         * Need to check if p is zero, 1, even, or odd and not one
         *
         * The first check is for zero if so n is 1 since any number^0 is 1. The
         * second check is for not 1 since all number^1 are themselves. The
         * third check is for not even and follows the path for fastPowering odd
         * numbers where you do n^(p/2) times n^(p/2) times n. The final else
         * catches all evens for the path of n^(p/2) times n^(p/2). Finally all
         * n's are mod'd by m and reassigned to n.
         *
         */
        if (pCopy.compareTo(zERO) == 0) {
            n.setFromInt(1);
        } else if (pCopy.compareTo(tWO) >= 0) {
            NaturalNumber temp = n.newInstance();
            if (!(isEven(pCopy))) {
                NaturalNumber nCopy1 = n.newInstance();
                nCopy1.copyFrom(n);
                pCopy.divide(tWO);
                powerMod(n, pCopy, m);
                temp.copyFrom(n);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber nMin1 = n.newInstance();
        nMin1.copyFrom(n);
        wCopy.copyFrom(w);
        //Power mods for rule 1(w^2 mod n =1) and checks it to the pass condition
        powerMod(wCopy, tWO, n);
        if (wCopy.compareTo(oNE) == 0) {
            ret = true;
        }
        nMin1.subtract(oNE);
        //Power mods for rule 2(w^(n-1) mod n != 1) and checks it to the pass condition
        powerMod(wCopy, nMin1, n);
        if (wCopy.compareTo(oNE) != 0) {
            ret = true;
        }
        return ret;
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            final int fIFTY = 50;
            NaturalNumber nMin4 = n.newInstance();
            NaturalNumber fOUR = new NaturalNumber1L(4);
            for (int i = 0; i < fIFTY; i++) {
                nMin4.copyFrom(n);
                nMin4.subtract(fOUR);
                /*
                 * randomNumber takes from [0,n] so I took [0,n-2] and added 1
                 * so my rand would be in [1,n-1] as specified in
                 * isWitnessToCompositeness's contract
                 */
                NaturalNumber rand = randomNumber(nMin4);
                rand.increment();
                rand.increment();
                ret = !isWitnessToCompositeness(rand, n);
            }
        }
        return ret;
    }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
         ...
         * the odd numbers only because all evens are not prime, until n is
         * likely prime
         */
        NaturalNumber tWO = new NaturalNumber1L(2);
        if (isEven(n)) {
